...
| `host` | `string` | **新增（向后兼容）**：当前运行机器的 hostname，运行时通过系统主机名接口动态获取，非硬编码 |
| `pid` | `number` | **新增（向后兼容）**：当前 Node 进程的进程号（整数），运行时动态取自 `process.pid`，非硬编码 |

> `uptime` 语义统一为「进程启动至今的秒数」，不与毫秒混用。调用方可按 `number` 稳定解析；新增字段不改变路由、HTTP 方法与既有字段。
>
> `host` 由运行时系统主机名接口（Node.js `os.hostname()`）动态获取，反映实际运行该服务的机器。
>
> `pid` 由运行时当前 Node 进程的 `process.pid` 动态获取，用于区分多实例/重启后的进程。
